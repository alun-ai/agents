...
> @agent-project-manager Begin implementation with the agent team. Ensure each agent uses ultrathink and sequential thinking to perform their tasks. Do not advance to the next issue until the current issue's code is implemented, reviewed, optimized, and verified with code reviews. Update the tracking issue with progress via @agent-github-manager. Enforce code reviews via @agent-code-reviewer before moving to the next issue. The plan is captured in the epic:  <github link>, the first ticket of actual work is <github subtask link>
> .


**Continue Each Issue:**

> @github-manager Update the github issues with progress. Re-read the issue to ensure the work stays on scope and is explicitly followed. @project-manager have the agents continue implementation of only the documented issue.  Coding tasks should be delegated to specialized agents.
